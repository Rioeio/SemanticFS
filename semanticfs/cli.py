...
def show_main_help_menu():
    ...
    core_table = Table(title="🔍 Core Search & Navigation (Stable)", expand=True, border_style="cyan")
    core_table.add_column("Command Syntax", style="bold green", width=28)
    core_table.add_column("Description & Action", style="white")

    core_table.add_row("sfind <query>", "Search files using natural language (e.g., sfind neural network ext:py in:docs)")
    core_table.add_row("sfind jump <query>", "Find target file & copy folder 'cd' command to clipboard")
    core_table.add_row("sfind recent", "Display 10 most recently modified files across monitored workspaces")
    core_table.add_row("sfind duplicates", "Identify high-similarity duplicate files across your drive")
    core_table.add_row("sfind tag <file> <note>", "Attach custom semantic notes & tags to any file for boosted search")
    core_table.add_row("sfind collection create/list", "Create or list Virtual Smart Collections (Zero disk modification risk)")
    console.print(core_table)

    daemon_table = Table(title="⚡ Daemon & Directory Management", expand=True, border_style="blue")
    daemon_table.add_column("Command Syntax", style="bold yellow", width=28)
    daemon_table.add_column("Description & Action", style="white")
    ...
    extras_table.add_column("Command Syntax", style="bold magenta", width=28)
    extras_table.add_column("Description & Action", style="white")

    extras_table.add_row("sfind model", "Launch 3D Movable 'Bad Apple!!' ASCII Neural Model Visualizer")
    extras_table.add_row("sfind commit <query>", "Search git commit messages across monitored repositories")
    extras_table.add_row("sfind train", "Fine-tune local neural embedding model on local files")
    extras_table.add_row("sfind onnx", "Export PyTorch model weights to quantized ONNX INT8 format")
    extras_table.add_row("sfind mount", "Mount virtual drive search directory for File Explorer integration")
    console.print(extras_table)
    console.print("\n[dim]Install optional extras: pip install -e \".[vision,ocr,onnx,train]\"[/dim]\n")
