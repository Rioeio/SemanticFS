...
SemanticFS includes a reproducible portable benchmark suite (`benchmarks/run_benchmarks.py`) evaluated across 40 held-out queries.

* **Top-1 Retrieval Accuracy**: **95.00%** (38/40 queries matched).
* **Mean Cold Search Latency**: **34.34 ms** (`p50 = 33.96 ms | p95 = 42.41 ms | p99 = 52.33 ms`).

For complete benchmark methodology and reproduction instructions, see [BENCHMARKS.md](docs/BENCHMARKS.md).
